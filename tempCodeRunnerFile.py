...
def main():
    ...
    #  Preload all features for all channels 
    ...
    #  Split into held-out test set (80-20 split)
    all_features = []
    all_labels = []
    ...
    for label_name, prefix in label_map.items():
        ...
        all_features.append(np.vstack([preloaded_features[label_name][ch+"_train"] for ch in channel_pool]).T)
        all_labels.append(np.array([0 if label_name=="Healthy" else 1]*len(train_idx)))

    X_train_full = np.vstack([np.hstack([preloaded_features[l][ch+"_train"] for ch in channel_pool]) 
                              for l in label_map])
    y_train_full = np.concatenate([np.array([0 if l=="Healthy" else 1]*preloaded_features[l][channel_pool[0]+"_train"].shape[0]) 
                                   for l in label_map])

    X_test = np.vstack([np.hstack([preloaded_features[l][ch+"_test"] for ch in channel_pool])
                        for l in label_map])
    y_test = np.concatenate([np.array([0 if l=="Healthy" else 1]*preloaded_features[l][channel_pool[0]+"_test"].shape[0])
                             for l in label_map])
    ...
    for combo in all_combinations:
        ...
        # Build train feature set for this combo
        X_combo = np.vstack([
            np.hstack([preloaded_features[label][ch+"_train"] for ch in combo])
            for label in label_map
        ])
        y_combo = np.concatenate([np.array([0 if l=="Healthy" else 1]*preloaded_features[l][combo[0]+"_train"].shape[0])
                                  for l in label_map])

        # --------- 3 iterations of 5-fold CV in parallel ----------
        ...
        iteration_results = Parallel(n_jobs=-1)(delayed(run_iteration)(seed) for seed in range(iterations))
        iter_f1_scores = [r[0] for r in iteration_results]
        iter_acc_scores = [r[1] for r in iteration_results]
        iter_conf_matrices = [r[2] for r in iteration_results]
    ...
    # --------- Retrain best combo on full train set and test on held-out test set ----------
    print("\nRetraining best combination on full train set...")
    X_best_train = np.vstack([
        np.hstack([preloaded_features[label][ch+"_train"] for ch in best_combo_name])
        for label in label_map
    ])
    y_best_train = np.concatenate([np.array([0 if l=="Healthy" else 1]*preloaded_features[l][best_combo_name[0]+"_train"].shape[0])
                                   for l in label_map])
    ...
    y_best_test = np.concatenate([np.array([0 if l=="Healthy" else 1]*preloaded_features[l][best_combo_name[0]+"_test"].shape[0])
                                  for l in label_map])
    ...
    # --------- Save best model, results, confusion matrix ----------
    ...
    # --------- Print ranked list of all combos ----------
